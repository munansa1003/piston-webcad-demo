export const meta = {
  name: 'catia-page-final-check',
  description: 'Re-verify the patched CATIA page data file with 3 independent checkers; a fixer edits the file until all pass',
  phases: [
    { title: 'Verify', detail: '3 checkers read the page file; a fixer edits it in place; up to 3 rounds' },
  ],
}

const DIR = '/tmp/claude-0/-home-user-piston-webcad-demo/d3b6b75d-e37c-54cf-808a-4ec885fd80b5/scratchpad'
const MAX_ROUNDS = 3
const FILE = `${DIR}/page-final2.json`
...
14. hundred.reasons: exactly 3 (amount of work, native files with feature history only made inside CATIA, OEMs name the CAD product) with numbers. oneLine: at most 2 short sentences that answer the whole question.
...
This is FINAL verification round ${round} of page data that already went through 4 rounds of checks and fixes. The PAGE DATA is the JSON file ${FILE}. Read it with python3 or jq (it is about 70 KB; load it with json.load and inspect the parts you need). Do NOT modify it. Set pass=true only if you find NO blocking issue. For each issue give the JSON path (e.g. "rulers[2].points[1].value"), the problem, and the exact fix (the replacement value or Korean text). Do not report the same problem twice. Only call something blocking if it breaks one of the hard rules or would actually mislead the reader; wording taste is minor.`
}
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  const checks = await parallel(CHECKERS.map((ch) => () =>
    agent(checkPrompt(ch, round), { label: `final-r${round}:${ch.key}`, phase: 'Verify', schema: CHECK, effort: 'high' })
  ...
  const fixNote = await agent(`${AUDIENCE}

You are the FIXER. The page data lives in the JSON file ${FILE}. First copy it to ${DIR}/page-final2.before-r${round}.json. Then apply every BLOCKING issue below by editing ${FILE} in place with python3 (json.load, change the exact fields, json.dump(obj, f, ensure_ascii=False, indent=1)). Verify each issue against the sources first; if an issue is itself wrong, leave that part unchanged and say why. Also apply MINOR issues when they are clearly right and cheap. Change nothing else. Keep the JSON valid, keep every field and its type, keep claimIds accurate. When done, re-open the file with json.load to prove it parses, and return a short plain-text list of what you changed.
...
MINOR ISSUES:
${minor.map((i, k) => `${k + 1}. [${i.checker}] ${i.path}: ${i.problem} -> FIX: ${i.fix}`).join('\n') || '(none)'}`, { label: `final-fix:r${round}`, phase: 'Verify', effort: 'high' })
  fixes.push({ round, note: fixNote })
}

return { history, lastChecks, fixes }
